...
public class BlisterActivity19 extends AppCompatActivity {
    ImageButton cargarBlisterSiguiente;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_blister19);

        cargarBlisterSiguiente = findViewById(R.id.boton_blister19);
    ...
    public void cargarBlister(View view) {
        if (cargarBlisterSiguiente.isClickable()) {
            ...
            if (fecha.equals(preferences.getString("tomaBlister_19", ""))) {
                ...
                Intent x = new Intent(this, BlisterActivity20.class);
                startActivity(x);
                finish();
                ...
                /// TOMA 20 DIA DE LA PILDRA

                SharedPreferences.Editor objEditor = preferences.edit();
                objEditor.putString("tomaBlister_20", fecha); // dado que a partir de ahora no será la pirmera vez, lo ponemos false
                objEditor.apply();
                objEditor.putInt("dia_20", 20); // dado que a partir de ahora no será la pirmera vez, lo ponemos false
                objEditor.apply();
            }
        }
    }
