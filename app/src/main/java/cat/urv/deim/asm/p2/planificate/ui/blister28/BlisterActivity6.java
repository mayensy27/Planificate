...
public class BlisterActivity6 extends AppCompatActivity {
    ...
    public void cargarBlister(View view) {
        if(cargarBlisterSiguiente.isClickable()){
            ...
            SharedPreferences preferences = getSharedPreferences("datos", Context.MODE_PRIVATE);
            SharedPreferences.Editor objEditor = preferences.edit();
            objEditor.putString("tomaBlister_7", fecha); // dado que a partir de ahora no será la pirmera vez, lo ponemos false
            objEditor.apply();
            objEditor.putInt("dia_7", 7); // dado que a partir de ahora no será la pirmera vez, lo ponemos false
            objEditor.apply();
        }
    }
}